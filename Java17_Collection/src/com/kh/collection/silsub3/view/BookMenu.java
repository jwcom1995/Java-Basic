package com.kh.collection.silsub3.view;

import java.util.Iterator;
import java.util.Scanner;
import java.util.Set;

import com.kh.collection.silsub3.controller.BookManager;
	...
	public void mainMenu() {
		...
		while (run) {
			...
			case 4:
				...
				if(bm.searchBook(title)==null) {
					System.out.println("조회한 글이 존재하지 않음");
				} else {
					System.out.println(bm.selectBook(title));
				}
				break;
			case 5:
				System.out.println("[5] 전체 출력");
				if(bm.selectAll().isEmpty()) {
					...
					Set<String> kSet=bm.selectAll().keySet(); 
					Iterator<String> itr = kSet.iterator();
					while(itr.hasNext()) {
						System.out.println(bm.selectAll().get(itr.next()));
					}
				}
				break;
			case 6:
